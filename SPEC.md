...
- **MA** — Aquifers. Lettered rather than numbered because it does **not** gate
  ...
  value. What the barrier measurement left open was not a barrier piece at
  all but the LEVEL a source carries below lambda — the dry sentinel and
  the ladder clamp — and that is now CLOSED too (item 6): both are as the
  spec states, measured through the barrier's Π rather than through a block
  readout, which cannot see below lambda at all. **`ChunkFiller` now
  ...
  a wider 64-chunk sweep. That residual was previously explained away as
  the level pieces above; it is not — item 6's change leaves it
  block-for-block unchanged, so it is still unattributed and wants a probe
  of its own. WITH the real 287-rule surface tree also running,
  ...
  carried). What remains is the fluid-type ceiling, that unattributed
  64-chunk residual, and ore veins (M3, untouched) — none of which is
