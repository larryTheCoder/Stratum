...
- **A `noise` field is a union, and narrowing it refused legal input (M4).**
  ...
  **Open question: how vanilla seeds a noise that has no name.** Both
  spellings load, and the graph carries the inline parameters, but only the
  named one can be *built*: a noise's generator is derived from the MD5 of
  its identifier, and an inline noise has no identifier. Nothing available
  here says what vanilla does instead, so the interpreter refuses such a node
  by name and says why, rather than substituting a derivation — the same
  choice, for the same reason, as `RandomSource::Legacy` above.
  `stratum validate` reports it as a warning: the pack is not wrong, this
  build is incomplete. Settling it needs the server, not reasoning: a
  datapack whose only difference from vanilla's is one inlined noise,
  generated by the vanilla server, and its terrain compared against
  candidates the way the blended noise was settled in M3.
