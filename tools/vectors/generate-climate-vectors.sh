#!/usr/bin/env bash
# Stratum — regenerate the 2D density pipeline vectors from cubiomes.
...
output="${1:-${repo_root}/tests/conformance/climate_vectors.inc}"
...
cc -O2 -std=c11 -ffp-contract=off -I"${work_dir}/cubiomes" \
    "${repo_root}/tools/vectors/climate_vectors.c" \
    ...
    -lm -o "${work_dir}/climate_vectors"

echo "==> writing ${output}" >&2
"${work_dir}/climate_vectors" > "${output}"
